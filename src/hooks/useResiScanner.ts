...
export const useResiScanner = ({ expedition, selectedKarung, formattedDate, allExpedisiDataUnfiltered }: UseResiScannerProps) => {
  ...
  const { data: recentResiNumbersForValidation, isLoading: isLoadingRecentResiNumbersForValidation } = useQuery<Set<string>>({
    queryKey: ["recentResiNumbersForValidation", todayFormatted], // Kunci kueri hanya untuk hari ini
    queryFn: async () => {
    ...
    gcTime: 1000 * 60 * 60 * 24 * 2, // Garbage collect setelah 2 hari
  ...
  const { data: allFlagNoExpedisiData, isLoading: isLoadingAllFlagNoExpedisiData } = useQuery<Map<string, any>>({
    queryKey: ["allFlagNoExpedisiData"], // Kunci kueri ini tidak berubah karena tidak ada filter tanggal
    queryFn: async () => {
      const data = await fetchAllDataPaginated(
        "tbl_expedisi",
  ...
  const handleScanResi = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(["recentResiNumbersForValidation", todayFormatted], (oldSet: Set<string> | undefined) => { // Updated query key
      ...
      console.error("Error during resi input (before IndexedDB save or during optimistic update):", error);
