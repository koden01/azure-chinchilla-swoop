...
export async function adjustResiCount(
  ...
  // console.log(`Current count for ${expedition} Karung ${karungNumber} on ${format(date, 'yyyy-MM-dd')}: ${currentCount}`); // Removed
  ...
  if (currentCount < targetCount) {
    ...
    // console.log(`Inserting ${numToInsert} records...`); // Removed
    ...
    // console.log(`Successfully inserted ${numToInsert} records.`); // Removed
    ...
    // console.log(`Deleting ${numToDelete} records...`); // Removed
    ...
    if (recordsToDelete && recordsToDelete.length > 0) {
      ...
      // console.log(`Successfully deleted ${numToDelete} records.`); // Removed
      return `Berhasil mengatur jumlah menjadi ${targetCount} dengan menghapus ${numToDelete} resi.`;
    } else {
      // console.log("No records found to delete."); // Removed
      return `Jumlah sudah ${currentCount}, tidak ada perubahan yang diperlukan.`;
    }
  } else {
    // console.log("Count is already 54. No changes needed."); // Removed
